...
> 3.1后， 系统默认直接增加了值为FLAG_EXCLUDE_STOPPED_PACKAGES的flag，导致即使是静态注册的广播接收器，对于其所在进程已经退出的app，同样无法接收到广播。
> 通过自定义广播权限来保护自己发出的广播。 在清单文件里receiver必须有这个权限才能收到广播。 需要定义权限，声明权限，这时接收者就能收到发送的广播。
> 广播的优先级，谁先注册谁优先级高， 并且对无序广播也生效
